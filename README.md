...
But wait — your debugger shows captured variables all the time. Pause on a
breakpoint and there they are, in the Scope panel. The debugger isn't guessing.
V8 exposes every function's captured scopes as an internal property called
...
runs the callback *before* `post` returns — so `dumps` is synchronous. Second,
the protocol only speaks in object ids and can't be handed a local value, so
values are passed through a global (`globalThis[Symbol.for('cloudpickle-js.hook')]`)
in both directions. Silly, but it works.
...
Yes, `with`. It resolves names through a real object, and that's precisely
what we need — the revived function gets *bindings*, not snapshots. Assigning
to a captured variable writes back to the scope object. That's why the counter
...
Serializing `huge` would be wasteful — and if a sibling closure captured a
socket, fatal. So a variable is kept only if its name appears in the
function's own source. (A regex checks this. `obj.small` doesn't count,
...
re-running `mixin(Base)` elsewhere would throw — or worse, quietly build a
different class. So the parent is serialized like any other captured value and
...
One more thing. Nothing above requires the function to live in *your* process.
Source, `[[Scopes]]`, properties — Chrome will answer the same questions about
a running tab. `demos/browser-capture.mjs` lifts a closure out of a web page
(which could never introspect itself) and keeps calling it in Node.

## What doesn't survive

- Native and bound functions — they have no source. Register them by
  reference instead.
...
- `this` in arrow functions and live module bindings — internal slots, not
  `[[Scopes]]`.
- `WeakMap`, `WeakSet`, pending promises. Rejected with an error, like locks
  and sockets in pickle.

And two rules: `loads` evaluates code, so only load payloads you trust. And it
needs Node ≥ 18 — browsers don't let a page use the inspector on itself.
